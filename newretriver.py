...
ANSWER_PROMPT = (
    "This question has been asked to retrieve information from a knowledge graph. The below question will get triplets from context got from the database. Use the information in the triples to answer the original question.\n\n"
...
def query_graph(question):
    ...
    if hasattr(result, 'content'):
        st.write("Response Content:", result.content)
    if hasattr(result, 'entities'):
        st.write("Extracted Entities:", result.entities)
    if hasattr(result, 'triples'):
        st.write("Retrieved Triples:", result.triples)
    
    # Assuming the result is an AIMessage object, you should be able to get the content
